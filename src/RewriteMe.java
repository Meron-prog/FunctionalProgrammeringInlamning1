...
public class RewriteMe {
        ...
        public List<String> getAllQuestionStringsBelongingACategory(Category category){
            return questions.stream().filter(q->q.getCategory().equals(category))
                    .map(q->q.getQuestionString()).collect(Collectors.toList());
        ...
        {
            return questions.stream().filter(q->q.getCategory().equals(c)).flatMap(q->q.getAllAnswers().stream()).distinct()
